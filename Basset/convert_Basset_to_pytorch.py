# Downlaoded from https://github.com/clcarwin/convert_torch_to_pytorch/blob/master/convert_torch.py and modified by Roman Kreuzhuber
...
def copy_param(m,n):
    #print("%s WEIGHTS: new model: %s, old model: %s"%(str(n), str(type(n.weight.data)), str(type(m.weight))))
    #print("%s BIAS: new model: %s, old model: %s"%(str(n), str(type(n.bias.data)), str(type(m.bias))))
    ...
    for m in module.modules:
        ...
        if name == 'SpatialConvolution' or name == 'nn.SpatialConvolutionMM':
            ...
            if hasattr(m, "ceil_mode"):
                ceil_mode = mp.ceil_mode
            n = nn.MaxPool2d((m.kW,m.kH),(m.dW,m.dH),(padW,padH),ceil_mode=ceil_mode)
            add_submodule(seq,n)
...
def lua_recursive_source(module):
    ...
    for m in module.modules:
        ...
        if name == 'SpatialConvolution' or name == 'nn.SpatialConvolutionMM':
            ...
            if hasattr(m, "ceil_mode"):
                ceil_mode = mp.ceil_mode
            s += ['nn.MaxPool2d({},{},{},ceil_mode={}),#MaxPool2d'.format((m.kW,m.kH),(m.dW,m.dH),(padW,padH),ceil_mode)]
        elif name == 'SpatialAveragePooling':
